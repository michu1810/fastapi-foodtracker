...
# Wyłącz natychmiastowe przerywanie skryptu w razie błędu
set +e
...
# Twoja oryginalna komenda uruchamiająca migracje
...
if [ $ALEMBIC_EXIT_CODE -ne 0 ]; then
    echo "BŁĄD KRYTYCZNY: Alembic zakończył działanie z kodem błędu $ALEMBIC_EXIT_CODE. Zatrzymuję uruchamianie aplikacji."
    # Możemy tu dodać 'exit $ALEMBIC_EXIT_CODE', jeśli chcemy zatrzymać cały kontener
fi

echo "--- Uruchamianie aplikacji Uvicorn ---"
# Twoja oryginalna komenda uruchamiająca aplikację
exec "$@"
