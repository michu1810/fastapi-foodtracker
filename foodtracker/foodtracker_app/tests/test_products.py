...
async def test_user_cannot_access_other_users_product(authenticated_client_factory, fixed_date):
	client_a = await authenticated_client_factory("user.a@example.com", "password123")
	create_response = await client_a.post("/products/create",
	                                      json={"name": "Produkt Usera A", "expiration_date": str(fixed_date),
	                                            "price": 5.0, "unit": "szt.", "initial_amount": 1})
...
async def test_use_product_more_than_available(authenticated_client_factory, fixed_date):
	client = await authenticated_client_factory("test.user.amount@example.com", "password123")
	create_res = await client.post("/products/create",
	                               json={"name": "Mleko", "expiration_date": str(fixed_date), "price": 3.0, "unit": "l",
	                                     "initial_amount": 2.0})
...
@pytest.mark.parametrize("payload, expected_detail_part", [
	...
	({"name": "Produkt", "expiration_date": "2025-06-17", "price": -1, "unit": "szt.", "initial_amount": 1},
	 "Input should be greater than 0"),
	({"name": "Produkt", "expiration_date": "2025-06-17", "price": 1, "unit": "szt.", "initial_amount": 0},
	 "Input should be greater than 0"),
])
...
async def test_use_exact_quantity_of_product(authenticated_client, fixed_date):
	create_response = await authenticated_client.post("/products/create",
	                                                  json={"name": "Awokado", "expiration_date": str(fixed_date),
	                                                        "price": 7.0, "unit": "szt.", "initial_amount": 2})
	assert create_response.status_code == 201
	product_id = create_response.json()["id"]
	use_response = await authenticated_client.post(f"/products/use/{product_id}", json={"amount": 2})
	assert use_response.status_code == 200
...
async def test_access_deleted_product_fails(authenticated_client, fixed_date):
	create_response = await authenticated_client.post("/products/create",
	                                                  json={"name": "Do usunięcia", "expiration_date": str(fixed_date),
	                                                        "price": 1.0, "unit": "szt.", "initial_amount": 1})
	assert create_response.status_code == 201
	product_id = create_response.json()["id"]
	delete_response = await authenticated_client.delete(f"/products/delete/{product_id}")
	assert delete_response.status_code == 204
	get_response = await authenticated_client.get(f"/products/get/{product_id}")
	assert get_response.status_code == 404


async def test_get_product_stats_with_real_data(authenticated_client_factory, fixed_date):
	client = await authenticated_client_factory("stats.user@example.com", "password123")
	res_a = await client.post("/products/create",
	                          json={"name": "Chleb", "expiration_date": str(fixed_date + timedelta(days=2)), "price": 5,
	                                "unit": "szt.", "initial_amount": 2})
	assert res_a.status_code == 201
	product_a_id = res_a.json()["id"]
	await client.post(f"/products/use/{product_a_id}", json={"amount": 1})
	res_b = await client.post("/products/create",
	                          json={"name": "Mleko", "expiration_date": str(fixed_date + timedelta(days=5)), "price": 3,
