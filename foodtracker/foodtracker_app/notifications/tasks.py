...
async def _notify_expiring_products():
    ...
    """
    print(f"[{datetime.now(timezone.utc).isoformat()}] Rozpoczynam sprawdzanie wygasających produktów...")
        ...
        try:
            ...
            for user in users:
                expiring_products_for_user = [
                    p for p in user.products
                    if p.current_amount > 0 and p.expiration_date <= expiration_threshold_date
                ]
                ...
                if expiring_products_for_user:
                    ...
                    print(f"Użytkownik {user.email} nie ma produktów do powiadomienia po filtrowaniu.")

        except Exception as e:
            import traceback
            print(f"Wystąpił błąd podczas wykonywania zadania notify_expiring_products: {e}")
            traceback.print_exc()

    print(f"[{datetime.now(timezone.utc).isoformat()}] Zakończono sprawdzanie wygasających produktów.")
