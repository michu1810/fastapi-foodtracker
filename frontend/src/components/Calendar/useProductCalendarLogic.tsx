...
const RENDER_DELAY = 50; // Opóźnienie w ms dla płynności animacji
...
export const useProductCalendarLogic = () => {
  ...
  // Widoczność „skorupy” modali
  const [isProductModalOpen, setIsProductModalOpen] = useState(false);
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isConsumptionModalOpen, setIsConsumptionModalOpen] = useState(false);

  // Opóźnione ładowanie ciężkiej zawartości
  ...
  const handleProductAdded = useCallback(() => {
    ...
    revalidateAllData();
  }, [revalidateAllData, handleCloseAddModal, t]);

  const handleAction = useCallback(
    async (action: 'use' | 'waste', productId: number, amount: number) => {
      if (!selectedPantry || !user) return;

      const serviceAction =
        action === 'use' ? productsService.useProduct : productsService.wasteProduct;
      ...
      try {
                ...
                (toastObj) => (
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="font-bold">{t('toasts.achievementUnlocked')}</p>
        ...
        const errorMessage = axios.isAxiosError(err)
          ? err.response?.data?.detail || t('serverError')
          : t('toasts.actionFailed');
        toast.error(errorMessage);
        revalidateAllData();
      }
    },
    [mutate, revalidateAllData, selectedPantry, user, t]
  );
  ...
  const handleUseProduct = (productId: number, amount: number) =>
    handleAction('use', productId, amount);
  const handleWasteProduct = (productId: number, amount: number) =>
    handleAction('waste', productId, amount);
  ...
  const handleDeleteProduct = useCallback(
    ...
    [selectedPantry, productsByDate, mutate, t]
  );

  const handleHideProduct = useCallback(
    (productToHide: Product) => {
      ...
      toast.success(t('toasts.hidden', { name: productToHide.name }));
    },
    [hiddenProductIds, t]
  );
    ...
    async (updatedProduct: Product) => {
      setProductsByDate((currentProducts) => {
        ...
        if (oldProduct && oldProduct.expiration_date !== updatedProduct.expiration_date) {
          ...
          newProductsState[oldDateKey] = (newProductsState[oldDateKey] || []).filter(
        ...
        const productsForNewDate = newProductsState[newDateKey]
          ? [...newProductsState[newDateKey]]
          : [];
  ...
  return {
    ...
    isProductModalContentLoaded,
    isAddModalContentLoaded,
    isConsumptionModalContentLoaded,
    productToConsume,
    actionType,
    handleDayClick,
    handleCloseProductModal,
