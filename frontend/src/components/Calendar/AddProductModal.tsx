            ...
            <div className="mt-4 grid grid-cols-1 gap-y-5">
                ...
                <div>
                    <Listbox value={selectedCategory} onChange={setSelectedCategory}>
                        <div className="relative">
                            ...
                            <Listbox.Options as={motion.ul} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-20">
                                {categoriesError && <div className="px-3 py-2 text-red-500">Błąd ładowania</div>}
                                {categories?.map((category) => (
                                    <Listbox.Option key={category.id} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-blue-100 text-blue-900' : 'text-gray-900'}`} value={category}>
                                        {({ selected }) => (
                                            <>
                ...
                <div>
                    ...
                    <div className="flex rounded-md shadow-sm">
                        <button type="button" onClick={() => setProductType('szt.')} className={clsx('relative inline-flex items-center justify-center w-1/2 px-4 py-2 text-sm font-medium rounded-l-md border border-gray-300 focus:z-10 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500', productType === 'szt.' ? 'bg-blue-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-50')}>Na sztuki</button>
                ...
                <div className="p-4 sm:p-6">
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        <div className="relative">
                            ...
                            <div onClick={() => setIsScannerVisible(true)} className="absolute inset-y-0 right-0 flex items-center pr-3 cursor-pointer text-gray-400 hover:text-blue-500" title="Skanuj kod kreskowy"><ScanBarcode size={20} /></div>
                        </div>
                        {isSearching && <p className="text-sm text-gray-500 mt-2">Szukam...</p>}
                        {error && <p className="text-sm text-red-500 mt-2">{error}</p>}
                        <div className="mt-2 max-h-48 overflow-y-auto">
                            {results.map((product) => (
                                <div key={product.id} onClick={() => handleProductSelect(product)} className="p-2 -mx-2 hover:bg-blue-50 cursor-pointer rounded-md">{product.name}</div>
                            ))}
                        </div>
