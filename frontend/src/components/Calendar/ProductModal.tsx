...
const ProductModal: React.FC<ProductModalProps> = ({ isOpen, date, products, onClose, onUseProduct, onDelete, onUpdate, onAddProduct, onWaste, onHide }) => {
    ...
    useEffect(() => {
        if (!isOpen) {
            const timer = setTimeout(() => setEditingProduct(null), 300); // Reset po animacji wyjścia
                    ...
                    <div key={product.id} className={`border rounded-lg p-3 sm:p-4 transition-shadow hover:shadow-md ${isFullyUsed ? 'bg-gray-100' : ''}`}>
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                            ...
                            <div className="flex gap-2 justify-end w-full sm:w-auto mt-3 sm:mt-0 sm:ml-4 flex-shrink-0">
                                {!isFullyUsed ? (
                                    <>
                                        <button onClick={() => handleEditClick(product)} title="Edytuj produkt" className="bg-yellow-400 hover:bg-yellow-500 text-white h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center rounded-full transition-transform hover:scale-110"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fillRule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clipRule="evenodd" /></svg></button>
                                        <button onClick={() => onUseProduct(product)} title="Zużyj produkt" className="bg-green-500 hover:bg-green-600 text-white h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center rounded-full transition-transform hover:scale-110"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg></button>
                                        <button onClick={() => onDelete(product)} title="Usuń cały wpis" className="bg-red-500 hover:bg-red-600 text-white h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center rounded-full transition-transform hover:scale-110"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                        <button onClick={() => onWaste(product)} title="Wyrzuć produkt" className="bg-gray-400 hover:bg-gray-500 text-white h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center rounded-full transition-transform hover:scale-110"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M9 2a1 1 0 011 1v1h3.5a.5.5 0 010 1H6a.5.5 0 010-1H9V3a1 1 0 011-1zm-3.707 4.293a1 1 0 011.414 0L10 9.586l3.293-3.293a1 1 0 111.414 1.414L11.414 11l3.293 3.293a1 1 0 01-1.414 1.414L10 12.414l-3.293 3.293a1 1 0 01-1.414-1.414L8.586 11 5.293 7.707a1 1 0 010-1.414z" /></svg></button>
                                    </>
                                ) : (
                                    <button onClick={() => onHide(product)} title="Ukryj wpis z widoku" className="bg-gray-400 hover:bg-gray-500 text-white h-8 w-8 sm:h-10 sm:w-10 flex items-center justify-center rounded-full transition-transform hover:scale-110"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                )}
                            </div>
                        </div>
                    </div>
