            ...
            <div className="flex justify-between items-center h-16">
              ...
              {user && (
                <div className="flex items-center space-x-2 sm:space-x-4">
                  ...
                  <Menu as="div" className="relative">
                    <Menu.Button className="flex items-center space-x-2 px-2 py-2 text-gray-700 hover:text-gray-900 rounded-full focus:outline-none transition transform hover:scale-105 active:scale-95">
                      <FaUserCircle className="text-2xl" />
                      <span className="hidden sm:inline transition-colors duration-150 group-hover:text-black">{user.email}</span>
                    </Menu.Button>
                    <Transition as={React.Fragment} enter="transition duration-150 ease-out" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="transition duration-100 ease-in" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                      <Menu.Items className="absolute right-0 mt-2 w-48 bg-white border shadow-lg rounded-md z-50 focus:outline-none">
                        <Menu.Item>{({ active }) => (<Link to="/profile" className={`block px-4 py-2 text-sm rounded focus:outline-none transition transform ${active? 'bg-gray-100 scale-105' : 'hover:bg-gray-50 hover:scale-[1.02]'}`}>Profil</Link>)}</Menu.Item>
                        <Menu.Item>{({ active }) => (<button onClick={logout} className={`w-full text-left px-4 py-2 text-sm text-red-600 rounded focus:outline-none transition transform ${active? 'bg-gray-100 scale-105' : 'hover:bg-gray-50 hover:scale-[1.02]'}`}>Wyloguj</button>)}</Menu.Item>
                      </Menu.Items>
                    </Transition>
                  </Menu>
